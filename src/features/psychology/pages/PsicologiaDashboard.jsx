...
export default function PsicologiaDashboard() {
  ...
  // Unique patients with session stats
  ...
  // Risk assessment (simple heuristic: patients with many pending = higher risk)
  const getRiskLevel = (patient) => {
    if (patient.pending >= 2) return "alto";
    if (patient.pending === 1) return "medio";
    return "bajo";
  };

  const mainTabs = [
    { id: "overview", label: "Vista General", icon: Brain },
    { id: "citas", label: "Mis Citas", icon: Calendar },
    { id: "patients", label: "Pacientes", icon: Users },
    { id: "notes", label: "Notas de Sesión", icon: ClipboardList },
  ];
    ...
    <div className="dashboard-container psych-dashboard">
      ...
      <div className="psych-hero">
        <div className="psych-hero-content">
          <div className="psych-hero-left">
            <div className="psych-hero-icon">🧠</div>
            <div className="psych-hero-text">
              <h1>Consulta de Psicología</h1>
              <p>{profile?.dependencies?.name || "Psicología"} — Bienestar mental y acompañamiento</p>
            </div>
          </div>
        ...
        <div className="psych-hero-stats">
          <div className="psych-hero-stat">
            <div className="psych-hero-stat-icon">⏳</div>
            <div className="psych-hero-stat-value">{stats.pending}</div>
            <div className="psych-hero-stat-label">Por Atender</div>
          </div>
          <div className="psych-hero-stat">
            <div className="psych-hero-stat-icon">📋</div>
            <div className="psych-hero-stat-value">{stats.confirmed}</div>
            <div className="psych-hero-stat-label">Confirmadas</div>
          </div>
          <div className="psych-hero-stat">
            <div className="psych-hero-stat-icon">✅</div>
            <div className="psych-hero-stat-value">{completionRate}%</div>
            <div className="psych-hero-stat-label">Tasa Éxito</div>
          </div>
        </div>
      </div>

      {/* ═══ TABS ═══ */}
      <div className="role-tabs" role="tablist" aria-label="Secciones de psicología">
        {mainTabs.map((t) => (
          <button
            key={t.id}
            role="tab"
            aria-selected={mainTab === t.id}
            className={`role-tab ${mainTab === t.id ? "active purple" : ""}`}
      ...
      <div className="admin-tab-content" role="tabpanel">
              ...
              <div className="psych-content-header" style={{ borderBottom: 'none', paddingBottom: 0 }}>
                {allPatients.length === 0 ? (
                  <div className="empty-state" style={{ width: '100%' }}>
                    ...
                    <p className="empty-state-description">Las citas asignadas a psicología aparecerán aquí.</p>
                  </div>
                ) : (
                  <div className="psych-patient-grid" style={{ width: '100%' }}>
                    {allPatients.map((p) => {
                      const risk = getRiskLevel(p);
                      return (
                        <div key={p.id} className="psych-patient-enhanced">
                          <div className="psych-patient-enhanced-header">
                            <div className="psych-patient-enhanced-info">
                              <div className="psych-patient-enhanced-avatar">{p.name[0]}</div>
                              <div>
                                <span className="psych-patient-enhanced-name">{p.name}</span>
                                <span className="psych-patient-enhanced-doc">{p.doc || "Sin documento"}</span>
                              </div>
                            </div>
                            <span className={`psych-risk-badge ${risk}`}>
                              <Shield size={10} /> {risk === 'bajo' ? 'Estable' : risk === 'medio' ? 'Seguimiento' : 'Urgente'}
                            </span>
                          </div>
            ...
            <div className="filter-pills" role="tablist" aria-label="Filtrar por estado">
              {STATUS_TABS.map(({ key, label }) => (
                <button
                  ...
                  className={`filter-pill purple ${filter === key ? "active" : ""}`}
            ...
            <div className="psych-appointments" role="tabpanel">
              {appointments.length === 0 ? (
                ...
                appointments.map((apt) => (
                  <div key={apt.id} className="psych-enhanced-row" data-status={apt.status}>
                    <div className="psych-enhanced-main" onClick={() => toggleExpand(apt.id)}>
                      <div className="psych-enhanced-info">
                        <div className="psych-enhanced-patient">
                          <div className="psych-enhanced-avatar">{apt.profiles?.full_name?.[0] || "?"}</div>
                          <div>
                        ...
                        <span className="psych-enhanced-status" data-s={apt.status}>
        ...
        {mainTab === "patients" && (
          <div className="psych-patients">
            {allPatients.length === 0 ? (
              ...
              <div className="psych-patient-grid">
                {allPatients.map((p) => {
                  const risk = getRiskLevel(p);
                  return (
                    <div key={p.id} className="psych-patient-enhanced">
        ...
        {mainTab === "notes" && (
          <div className="psych-content-card">
            <div className="psych-content-header">
              <h3 className="psych-content-title">
                <ClipboardList size={18} /> Historial de Notas
              </h3>
            ...
            <div style={{ padding: '20px 24px' }}>
              {notesHistory.length === 0 ? (
                <div className="empty-state">
                  ...
                  <p className="empty-state-description">Las notas de sesión aparecerán aquí cuando las agregues a las citas confirmadas.</p>
                </div>
              ) : (
                    ...
                    <div key={apt.id} className="enhanced-note-card">
                      <div className="enhanced-note-header">
                        <div className="enhanced-note-patient">
                          <div className="psych-enhanced-avatar" style={{ width: 36, height: 36, fontSize: 14 }}>
                      ...
                      <div className="enhanced-note-body purple">
                        <StickyNote size={14} style={{ color: '#7c3aed', flexShrink: 0, marginTop: 2 }} />
      ...
      {notesModal && (
        <div className="modal-overlay" onClick={() => setNotesModal(null)} role="dialog" aria-modal="true">
          <div ref={modalRef} className="modal-content enhanced-modal psych-notes-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2 className="modal-title">
                <StickyNote size={18} /> Notas de Sesión
              </h2>
            ...
            <div className="modal-body">
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                <div className="psych-enhanced-avatar" style={{ width: 44, height: 44 }}>
              ...
              <textarea
                ...
                placeholder="Describe la evolución del paciente, observaciones clínicas, recomendaciones..."
                rows={6}
                autoFocus
              />
